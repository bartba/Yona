...
Wraps the Silero VAD V5 ONNX model with speech/silence state tracking and
...
class VoiceActivityDetector:
    ...
    Maintains LSTM state across chunks so the model's recurrent memory
    persists throughout a conversation turn.  Call :method:`reset` when
    starting a new turn.
    ...
    def __init__(self, cfg: Config, bus: EventBus) -> None:
        ...
        self._silence_duration: float = cfg.get("vad.silence_duration", 1.5)
        ...
        # Silero V5 LSTM state — each tensor is [2, 1, 128]
    ...
    def process_chunk(self, chunk: np.ndarray) -> float:
        ...
        x = audio.reshape(1, self._chunk_size)
